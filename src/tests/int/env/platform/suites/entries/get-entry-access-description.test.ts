...
describe('Get entry access description', () => {
    ...
    test('Returns access description without workbook access (only tenant is checked)', async () => {
    ...
    test('The /meta endpoint denies the same user (contrast)', async () => {
        await auth(request(app).get(`${routes.entries}/${dashEntryId}/meta`)).expect(403);
    });
